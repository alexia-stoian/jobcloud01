...
must_haves:
  truths:
    - "After the top-3 results are built, the sourcing POST creates one SourcingSession and, for every shown candidate whose displayed fitPercent >= 60, generates <=5 gap questions and queues a SourcingCandidate with fitBefore = that displayed fitPercent."
  ...
  artifacts:
    ...
    - tests/integration/sourcing-session-readback.test.ts
  key_links:
    - "sourcing POST: after results.sort, createSourcingRun(...) then Promise.all over results.filter(r => r.fitPercent >= 60) calling generateGapQuestions + queueCandidateQuestions(fitBefore=r.fitPercent)."
...
- `const run = await createSourcingRun({ recruiterUserId: gate.session.user.id,
  needsSnapshot: needs, roleLabel: needs.role ?? null })`.
- `const qualifying = results.filter((r) => r.fitPercent >= 60)`.
- `await Promise.all(qualifying.map(async (r) => { const scored = topResults.find(...);
  const questions = await generateGapQuestions(needs, r); if (questions.length === 0) return;
  await queueCandidateQuestions({ sessionId: run.id, candidateUserId: r.userId,
  fitBefore: r.fitPercent, questions }); }))`.
...
- **T1 — Generate + persist on the sourcing run; read-back endpoint**
  ...
  - **Action**: In `sourcing/route.ts`, add the §2 generation hook after `results.sort(...)`:
    import `createSourcingRun`, `queueCandidateQuestions` from `@/lib/sourcing/session-dal`
    and `generateGapQuestions` from `@/lib/sourcing/questions`; create the run keyed to the
    admin (`gate.session.user.id` — confirm the `requireAdmin` return shape exposes the user
    id, else read it from the gate session), then `Promise.all` over the `>= 60` results
    generating and queueing (fitBefore = `result.fitPercent`). Keep the block inside a
  ...
  - **Verify**:
    ...
    Plus: grep confirms `sourcing/route.ts` calls `createSourcingRun` and gates on
    `>= 60`, and `sourcing/session/route.ts` calls `requireAdmin` before any
    `readBackForRecruiter`/db call.
  - **Done**: A run + queued questions are persisted for every >=60% shown candidate, and an
    admin-gated read-back endpoint returns before->now + Q&A with no correctness leakage.
...
- **T2 — Sourcing card: Profile slide-over + Q&A / before->now section**
  ...
  - **Verify**:
    ...
    `admin-scrim`, and fetches `/api/admin/sourcing/session`.
...
- **T3 — EN/DE/FR keys + read-back integration test**
  ...
  - **Done**: All three locales carry the recruiter + candidate keys, and the integration test
    proves the read-back is admin-gated and leaks no correctness data.
...
1. **Profile opens the Admin profile without navigating away.** — T2 button + slide-over. →
   SGQ-01
2. **Every >=60% shown candidate gets <=5 queued gap questions at run time.** — T1 §2 hook
   gated on `fitPercent >= 60`. → SGQ-02
...
- [ ] Sourcing POST persists a run + queued questions for every >=60% shown candidate
      (fitBefore = displayed `fitPercent`), degrading silently and never breaking the response.
